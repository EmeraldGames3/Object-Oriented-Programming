...
int *findSubarrayWithLongestSum(const int numbers[], int &len) {
    ...
    int j = 0;
    //Here we store the subarray with the biggest sum in printAllRelativePrimes variable so that we can return it
...
int main() {
    ...
    std::cout << "Enter printAllRelativePrimes number ";
    std::cin >> n;
